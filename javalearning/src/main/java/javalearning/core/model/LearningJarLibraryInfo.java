		...
		for (Path p : paths) {
			if (Files.isDirectory(p)) {
				...
				if (p.toString().endsWith(".class")) {
					...
					// index 0 is the modules dir, index 1 is the module name (eg. java.base), so we
					// start at index 2
					for (int i = 2; i < p.getNameCount(); i++) {
						...
						if (!p.getName(i).toString().endsWith(".class")) {
							...
							fqName.append(p.getName(i).toString(), 0,
									p.getName(i).toString().lastIndexOf(".class"));
						}
					}
...
//                System.out.println(": " + b.length);
	...
	public int compareTo(LibraryInfo o) {
		...
		if (o instanceof LearningJarLibraryInfo) {
			LearningJarLibraryInfo other = (LearningJarLibraryInfo) o;
